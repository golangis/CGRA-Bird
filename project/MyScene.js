...
export class MyScene extends CGFscene {
  ...
  init(application) {
    ...
    this.treeShader = new CGFshader(this.gl, "shaders/billboard.vert", "shaders/billboard.frag");

    // trees info

    let treeData = [
      [[21.21, 5.67], 1], [[10.82, 51.61], 1], [[18.84, 106.9], 0], [[20.88, 161.6], 0], [[14.72, 212.2], 0],
      [[3.22, 251.08], 0], [[18.06, 316.65], 1], [[23.7, 364.62], 1], [[62.36, 22.01], 0], [[65.18, 61.35], 1],
      [[51.3, 117.58], 0], [[50.44, 172.44], 0], [[62.36, 213.52], 0], [[72.02, 260.34], 0], [[58.89, 319.38], 0],
      ...
      [[315.1, 355.77], 1], [[374.32, 17.41], 0], [[373.63, 50.61], 1], [[370.48, 120.8], 1], [[350.16, 165.27], 1],
      [[360.21, 215.45], 1], [[359.4, 255.81], 1], [[352.02, 311.06], 0], [[362.17, 365.46], 0], 
    ]
    ...
    for (let i = 0; i < treeData.length; i++) {
      if (treeData[i][1]) {
        this.trees.push(new MyTreeGroupPatch(this, treeData[i][0][0] - 200, treeData[i][0][1] - 200))
      } else {
        this.trees.push(new MyTreeRowPatch(this, treeData[i][0][0] - 200, treeData[i][0][1] - 200))
      }
    }
